...
sk_sp<SkImage> KawaseBlurDualFilter::generate(SkiaGpuContext* context, const uint32_t blurRadius,
    ...
    sk_sp<SkSurface> surfaces[kMaxSurfaces] =
            {filterPasses >= 0 ? makeSurface(1 * mInverseInputScale) : nullptr,
             ...
             filterPasses >= 4 ? makeSurface(16 * mInverseInputScale) : nullptr,
             filterPasses >= 5 ? makeSurface(32 * mInverseInputScale) : nullptr};
    ...
    for (int i = 0; i < filterPasses; i++) {
        ...
        sumSquaredR += powf(powf(2.0f, i) * alpha * kDownWeights[1 + i] / mInputScale, 2.0f);

        // Add upscale weight
        int upIndex = (filterPasses - 1) - i;
        sumSquaredR += powf(powf(2.0f, i + 1) * alpha * kUpWeights[upIndex] / mInputScale, 2.0f);
    }
